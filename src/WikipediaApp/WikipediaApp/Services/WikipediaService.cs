...
namespace WikipediaApp
{
  public class WikipediaService
  {
    ...
    public async Task<Article> GetArticle(Uri uri, bool sectionsCollapsed)
    {
      ...
      if (string.IsNullOrEmpty(title))
      {
        ...
        return await GetArticle(header, sectionsCollapsed);
      }

      try
      {
        return await parseApi.FetchArticle(language, sectionsCollapsed, uri, title: title);
      }
      catch (Exception ex)
      {
    ...
    public async Task<Article> GetArticle(ArticleHead article, bool sectionsCollapsed)
    {
      try
      {
        return await parseApi.FetchArticle(article.Language, sectionsCollapsed, article.Uri, title: article.Title, pageId: article.Id);
      }
      catch (Exception ex)
      {
        HockeyClient.Current.TrackException(ex);
    ...
    public async Task<Article> RefreshArticle(Article article, bool sectionsCollapsed)
    {
      try
      {
        return await parseApi.FetchArticle(article.Language, sectionsCollapsed, article.Uri, article.PageId, article: article);
      }
      catch (Exception ex)
      {
        HockeyClient.Current.TrackException(ex);
